...
| Responsive behavior | Verified for Hero, Event Glance, Timeline, Prize Display, About, Challenges, Rules | Real Playwright/Chromium screenshots at 375/768/1440px each round, not just class-name inspection | A real bug was caught this way once (Prize Display's featured-card text clipping at a 768px 3-column layout) and fixed before shipping |
...
| AnimatedGridPattern reseed bug | Fixed | `components/ui/animated-grid-pattern.tsx` | Was reseeding all squares on every container resize, not just on mount (DEC-016) — visible as the whole grid background "flashing" whenever FAQ's accordion changed the page's height. Root cause was a genuine resize (the grid's absolutely-positioned SVG spans the whole auto-height page wrapper), not a wasted re-render; fixed with a `hasSeededRef` guard rather than `React.memo`. The original `react-hooks/set-state-in-effect` lint suppression is fully removed, not just narrowed — confirmed by running lint after the fix |
| TextReveal (text-entrance primitive) | Active | `components/ui/text-reveal.tsx`, applied to `/about`, `/challenges`, `/rules`'s headings/paragraphs | Extracts Hero/Timeline's independently-duplicated fade+rise pattern into one reusable component (DEC-012); adds word-level stagger for headings. Hero/Timeline's own existing implementations were **not** retrofitted, and neither were FAQ/Register — open scoping question, not yet decided |
...
| IconCard | Active | `components/ui/icon-card.tsx`, used on `/rules` | Icon + title + description card for parallel categorical grids (DEC-014) — a first attempt at reusing an "existing" IconCard turned out not to exist anywhere in the codebase; built fresh instead |
| VerticalAccentText | Active | `components/ui/vertical-accent-text.tsx`, used on `/prizes`, `/faq`, `/register` | Extracted from Prize Display's inline copy once `/faq` needed the same device (DEC-015) — a request to "reuse the version already on About" turned out to be a mix-up too (it was only ever on `/prizes`) |
| TicketPass | Active | `components/public/ticket-pass.tsx`, used on `/register` | Hand-built boarding-pass/ticket visual (DEC-017) — searched npm/Aceternity/shadcn.io/Magic UI first, nothing existed to reuse. Live "Departure" countdown field computed client-side (useEffect + useRef guard, avoiding an SSR/hydration mismatch on a statically-prerendered page) |
| FormField / InterestForm | Active | `components/ui/form-field.tsx`, `components/public/interest-form.tsx`, used on `/register` | First real Form Field (DEC-018). Name+email interest-capture, not full registration — stub `handleRegistrationSubmit` handler (exported, TODO-commented for Satyendra/`origin/sback` to wire the real Supabase insert), deliberately not named `submitRegistration` to avoid colliding with that branch's own full registration action. Success/error states both verified end-to-end in a real browser |
| Status / Notification Banner | Active | `components/public/notification-banner.tsx`, used on `/rules` | First real implementation of a previously spec-only registry entry (DEC-014) |
...
| `/rules` | Done | `app/rules/page.tsx` | Verified at 375/768/1440px | All confirmed facts only, copy unchanged from prior version | Grid backdrop (DEC-010), `TextReveal`, a 3x2 `IconCard` grid (not a sequence/timeline motif, kept visually distinct from `/timeline` and `/challenges`), and the page's first real Status/Notification Banner usage (DEC-014). A real production build error (Lucide icon components passed as props from a Server to a Client Component — functions aren't serializable) was caught and fixed before shipping |
| `/faq` | Done | `app/faq/page.tsx` | Verified at 375/768/1440px | All confirmed facts only, copy unchanged from prior version | Grid backdrop (DEC-010, previously missing entirely — a real gap, not a stylistic choice), `TextReveal`, numbered (01–09) accordion, `VerticalAccentText` (newly extracted, see above). All 9 answers cross-checked against `tbd.md` before the redesign started — the 3 specifically flagged were already honestly hedged |
| `/register` | Shell Done, Interest Form Done, Full Registration Not Started | `app/register/page.tsx`, `components/public/ticket-pass.tsx`, `components/public/interest-form.tsx` | Verified at 375/768/1440px | All confirmed facts only | Visual shell rebuilt around a boarding-pass/ticket metaphor (DEC-017) — grid backdrop, `TextReveal`, `VerticalAccentText`. A real, working name+email interest-capture form (DEC-018) sits below the ticket, with a stub submit handler (not wired to Supabase — that's `origin/sback`'s job) and both success/error states built and verified. The *full* competition registration form/workflow remains Not Started, gated on `tbd.md` — see Section 6 |
...
### Avoid
- Do not start on `/register`'s full competition-registration form fields, team/participant/admin work, or any Supabase table — all of that is either gated on unresolved `tbd.md` decisions or sequenced into later phases. (This Phase 1 "Avoid" note predates DEC-018's narrower, explicitly-authorized exception: a name+email *interest-capture* form, deliberately scoped smaller than full registration and not wired to Supabase — see that decision and Section 5's `InterestForm` row.)
