...
| Subtle circuit/grid line texture | Site-wide default page backdrop (via `PageBackdrop`), on every route including Hero (beneath the ignition glow there specifically) — per `context/decisions.md` DEC-010, which supersedes DEC-006's narrower Hero/Timeline/Prizes-only scope | N/A — this is now the default, not an exception | Reinforces the technology-forward feel consistently, at very low opacity so it never competes with text contrast |
...
| Decorative icons/illustrations | Not used as decoration, with two narrow, named exceptions: (1) small functional icons naming a Timeline node's phase (registration/prelims/finale), always paired with the node's text label, never standing alone, per `context/decisions.md` DEC-006 — DEC-007's bespoke `/prizes` wireframe gem was retired per DEC-009, so that exception no longer applies; (2) two bespoke network-graphic illustrations (`IndiaNetworkMap`, `GlobalNetworkGlobe`) on `/about` only, per DEC-010, sharing one consistent dot/arc/glow visual system at two scales, `primary`/`border-light`/`ember` only | Anywhere else in the product | The system relies on color, type, and restraint rather than illustrative decoration; both remaining exceptions are scoped to the one page/element named, not a general license |
