...
- **Relevant routes:** `/about`, `/challenges`, `/rules`, `/faq`, `/register`. Not yet retrofitted onto Hero/Timeline's own existing hand-rolled implementations — see `context/decisions.md` for the reasoning and `progress-tracker.md` for what's still open.
...
### VerticalAccentText
- **Status:** Active — `components/ui/vertical-accent-text.tsx`. Originally built inline for Prize Display (DEC-009); extracted here once `/faq` wanted the same device (DEC-015), rather than a second hand-copied instance.
- **Purpose:** A stacked, small-caps vertical word column — purely atmospheric/decorative, desktop-only.
- **Composition:** the four words of `project-overview.md`'s confirmed theme string ("Scalable Innovations for Next-Gen India" — already used verbatim in Hero), each rendered via `[writing-mode:vertical-rl]`. Fixed content — not a generic "pass any words" component — specifically so no page introduces new invented phrasing by reaching for this component with different words; if a future use case genuinely needs different text, that's a product decision belonging in `tbd.md`/`decisions.md` first, not a prop.
- **Responsive behavior:** `hidden lg:flex` — hidden below `lg` (1024px). A narrow single-column layout has no natural place for a decorative side column; both current consumers (Prize Display, FAQ) already reflow to a single column at that width.
- **Accessibility requirements:** `aria-hidden="true"` — decorative only, conveys no information not already present elsewhere on the page.
- **Token/rule dependencies:** `text-muted` only.
- **Relevant routes:** `/prizes`, `/faq`, `/register`.
...
- **Relevant routes:** `/timeline` only. `/prizes` no longer uses this component — it now has its own bespoke two-column header built into `PrizeDisplay`, per `context/decisions.md` DEC-008. `/about`, `/challenges`, `/rules`, `/faq`, and `/register` all exist now but each uses its own centered hero-style intro (originally built by a teammate for the first four, kept and extended rather than migrated to `PageHeader`; `/register`'s is new, part of its ticket-metaphor rebuild, DEC-017) instead of this component — a pre-existing inconsistency with this entry's "standard pattern" framing, not yet reconciled; see `progress-tracker.md`.
...
- **Composition:** header block (eyebrow "Prizes" + `h1` "Rewarding what matters" + supporting copy, the confirmed ₹6,00,000/three-tiers/grand-finale statement + a "Register Now" CTA → `/register`) — a named, page-specific exception to `ui-rules.md`'s Page Headers pattern, per DEC-008/DEC-009. Below it: a full-width row containing the waveform+markers (a hand-authored three-peak SVG path, `Marker`s positioned on each peak — see Prize Card) and `VerticalAccentText` (see its own entry above) at `lg`+. Originally an inline copy of the accent-column markup; extracted into the shared `VerticalAccentText` component once `/faq` wanted the same device (DEC-015) — this page was refactored to consume it, re-verified unchanged after.
...
### Ticket Pass
- **Status:** Active — `components/public/ticket-pass.tsx`, per `context/decisions.md` DEC-017
- **Purpose:** `/register`'s hero visual — a hand-built boarding-pass/event-ticket shape, deliberately distinct from every other page's approach. Before building, npm, Aceternity UI's full component list, shadcn.io's 59-component list (covers Magic UI too), and a targeted registry/21st.dev search were all checked — nothing resembling a ticket-stub pattern exists anywhere, so this is hand-drawn decorative geometry (no "correct" real-world answer to get wrong, unlike the India map, DEC-011).
- **Composition:** a card split by a dashed perforation into a main "pass" section (PASS label, "Awaiting Confirmation" status, Event Dates/Venue/Departure/Class fields) and a smaller stub (three manifest-style line-items: icon + label + description, reusing the "What You'll Need" copy verbatim). Die-cut notches — at all four card corners, and at the two points the perforation meets the card edge — are `bg-background`-colored circles overlapping the border, not a hardcoded color or SVG clip-path, so they stay correct if that token ever changes.
- **Responsive behavior:** the perforation's *orientation* changes, not just its scale — vertical (main/stub side by side) at `lg`+, horizontal (stub stacked beneath) below `lg`, since a vertical stub squeezed into a narrow column would leave an awkwardly thin sliver. Both DOM structures always render; Tailwind responsive classes toggle visibility, matching this project's standard pattern for structural responsive differences (avoids hydration risk from viewport-conditional rendering). Verified at 375/768/1440px — no horizontal overflow at any width.
- **The "Departure" countdown field**, specifically: computed client-side in a `useEffect` gated by a `useRef` one-time-init guard (the same pattern that fixed `AnimatedGridPattern` in DEC-016 — `react-hooks/set-state-in-effect` flagged the naive synchronous version for the identical underlying reason). This page is statically prerendered, so computing `Date.now()` synchronously during render would bake in the build-time value on the server while the client computes a different, correct value at hydration — a hydration mismatch. The server/first-paint render shows a placeholder (`—`); the real day-count fills in after mount. Verified: 107 days as of this build, independently recomputed via a standalone script, not eyeballed from the UI.
- **No functional CTA anywhere on the card itself** — registration isn't wired up, so nothing on the ticket is styled as a clickable action; status is plain badge/text. The page's actual interactive element is `InterestForm` (see below), a separate section below the ticket — the ticket stays a preview/illustration, the form is what a visitor can actually do today.
- **Accessibility requirements:** entrance is a one-time `initial`/`animate` rise+scale (not scroll-triggered — this is the page's hero-equivalent content, always above the fold on load, matching Hero's own `animate="show"` pattern rather than `whileInView`), with internal fields staggering in via variant propagation. Gated on `prefers-reduced-motion` via `MotionConfig`'s `reducedMotion="user"`, no `useReducedMotion()` structural branch. Re-verified the DEC-016 grid-reseed fix holds on this page specifically (even with no accordion here): grid-pattern square positions captured after the ticket's entrance animation and countdown effect settle, then again after a further wait, came back byte-identical — confirms neither the entrance transform nor the countdown's `setState` triggers a reseed.
- **Token/rule dependencies:** `surface-secondary`/`border`/`background` (card + notches), `primary` (pass label, icons), `warning`/`warning-light` (status badge — accurately reflects "not yet confirmed," not a fabricated "open" state), `border-light` (perforation line), `text-primary`/`text-secondary`/`text-muted`.
- **Relevant routes:** `/register` only.

### InterestForm
- **Status:** Active — `components/public/interest-form.tsx`, per `context/decisions.md` DEC-018
- **Purpose:** `/register`'s actual functional element — a name+email interest-capture form ("notify me when registration opens"), scoped deliberately narrower than full competition registration since that workflow is unconfirmed (`tbd.md`). Composes `Card` + `FormField` + `Input` + `Button` into the "Form Section" pattern (see that entry above) — no separate component was extracted for the composition itself.
- **Composition:** eyebrow + heading + a sentence explicitly distinguishing "registering interest" from "confirming a spot" (directly so `TicketPass`'s "Awaiting Confirmation" badge, on the section above, isn't misread as describing a personal submission status), an inline `error`-variant `NotificationBanner` shown only when `status === "error"`, then the two `FormField`s (Name, Email) and a submit `Button` using its built-in `loading` state while `status === "submitting"`.
- **States:** `idle`, `submitting` (inputs disabled, button shows its spinner), `success` (replaces the whole form with a `success`-toned confirmation card — check icon + generic message, never a specific claimed next step, per `ui-rules.md`'s Forms section), `error` (banner above the still-filled-in form, so the visitor can retry without re-typing).
- **Validation:** hand-rolled (required name, required + regex-checked email) rather than Zod — Zod is architecturally adopted per `library-docs.md` but isn't installed on this branch (it lives on the backend integration branch); installing a dependency for two simple client checks that get properly re-validated server-side once real backend logic lands would be exactly the kind of one-off-convenience addition `library-docs.md` advises against.
- **The submit handler, specifically:** `handleRegistrationSubmit(data: { name, email }): Promise<{ success: boolean }>`, exported and documented with a `TODO` naming exactly what it needs to become (a Supabase insert into a new `interest_signups` table) and who owns that. Deliberately **not** named `submitRegistration` — that name is already used by a full, auth-gated competition-registration server action on the backend integration branch (`origin/sback`, `actions/register.ts`, targeting a `registrations` table) — reusing it here for a smaller, different-shaped flow would have been a real footgun for whoever wires this up next, not just a naming preference.
- **Accessibility requirements:** `noValidate` on the `<form>` (validation is handled entirely by this component's own logic, not native browser validation UI, so error messaging stays consistent with the rest of the design system), each `Input` carries `aria-invalid`/`aria-describedby` wired to its `FormField`'s error text.
- **Token/rule dependencies:** see `Form Field`/`Status/Notification Banner`/`Card`/`Input`/`Button` above — no new tokens.
- **Relevant routes:** `/register` only.
- **Notes:** Verified end-to-end in a real browser: empty-submit validation, invalid-email-format validation, a full valid submission reaching the success state with the stub handler's exact `console.log` output captured, and the error state verified by temporarily forcing the stub to return `{ success: false }`, screenshotting it, then reverting — confirmed via `grep` and a full re-run of the success path afterward, not assumed correctly reverted.
...
### Form Field
- **Status:** Active — `components/ui/form-field.tsx`. First real implementation of this previously spec-only entry, built for `/register`'s interest-capture form (DEC-018).
- **Purpose:** The reusable label + input/textarea/select + helper/error-text unit.
- **Composition:** Form label (with a `text-secondary`, non-semantic required asterisk) + one of (Input/Textarea/Select/Checkbox/Radio, rendered as `children` — this component doesn't render the input itself) + helper or error text below.
- **States:** default, focus, error, disabled. Error/focus/disabled all live on the input primitive itself (`Input`'s `aria-invalid`, etc.) — `FormField` only swaps its own helper text for error text and doesn't otherwise restyle based on state.
- **Do not use when:** N/A — this is the generic shell for any labeled field; use it whenever a field needs a label + optional required marker + helper/error text.
- **Token/rule dependencies:** form-label typography, form-help-text typography, `error`.
- **Accessibility requirements:** the caller is responsible for connecting `id`/`aria-invalid`/`aria-describedby` on the actual input element to this component's `id`/error text — kept explicit rather than auto-injected via `cloneElement`, so the field↔input relationship stays visible in the calling code. See `InterestForm` below for the pattern.
...
### Form Section
- **Status:** Active (as a composition pattern, not a separate component) — `/register`'s `InterestForm` (`components/public/interest-form.tsx`) composes `Card` + `FormField`s + `Button` directly, matching this entry's own described shape. No standalone `FormSection` component was extracted, since "Card containing Form Fields + submit Button" was already documented here as a composition, the same way "Timeline Section"/"Prize Section" are page-level patterns rather than components — see DEC-018.
...
**Reusable composition:** `TicketPass` (illustrative preview, DEC-017) → `InterestForm` (Form Section: Card containing Form Fields + submit Button, DEC-018) → Status/Notification Banner on error.
**Product-specific — resolved for interest-capture only, still NOT defined beyond that:** the live form on `/register` today is scoped to name+email interest-capture (DEC-018), not full competition registration — that remains unresolved. Whether full registration requires account creation before or after, what fields it needs beyond name/email, and what happens post-submission for the *real* registration flow are all still open per `tbd.md`'s Registration decision entry.
**Applies to:** `/register`.
...
| TextReveal | Primitive | Active | One-time fade+rise text entrance (word-stagger or block) | `/about`, `/rules`, `/challenges`, `/faq`, `/register` |
| VerticalAccentText | Primitive | Active | Stacked small-caps vertical theme-word column | `/prizes`, `/faq`, `/register` |
...
| Ticket Pass | Composed | Active | Hand-built boarding-pass/ticket hero visual, live countdown field | `/register` only |
| InterestForm | Composed | Active | Name+email interest-capture form, stub submit handler | `/register` only |
...
| Form Field | Composed | Active | Label + input + helper/error unit | `/register` |
| Form Section | Composed | Active (composition, not a component) | Grouped Form Fields + submit | `/register` |
